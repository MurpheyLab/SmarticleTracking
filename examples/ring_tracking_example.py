...
def get_ring_center(trck, ring_ids):
    ring_tag_xy = [obj.x[:1] for obj in trck.tracking_objects if obj.id in ring_ids]
    ring_center = sum(ring_tag_xy)/len(ring_tag_xy)
    r = 10
    color = (0,255,0)
    thick = -1
    cv2.circle(trck.frame, (ring_center[0],ring_center[1]),r, color, thick)
    return ring_center
